...
def Model(prompt: str = 'test'):
    ...
    # --- BEGIN FIX ---
    # Check for exact direct automation commands inside prompt
    direct_commands = ['next image', 'previous image', 'close image', 'delete images']
    cleaned_prompt = prompt.strip().lower()

    for command in direct_commands:
        if command in cleaned_prompt:
            return [command]
    # --- END FIX ---
